...
public final class CustomActivityOnCrash {
    ...
    public static void install(@Nullable final Context context) {
        try {
            if (context == null) {
                LogUtils.e("Install failed: context is null!");
            } else {
                //INSTALL!
                final Thread.UncaughtExceptionHandler oldHandler = Thread.getDefaultUncaughtExceptionHandler();

                if (oldHandler != null && oldHandler.getClass().getName().startsWith(CAOC_HANDLER_PACKAGE_NAME)) {
                    LogUtils.e("CustomActivityOnCrash was already installed, doing nothing!");
                } else {
                    if (oldHandler != null && !oldHandler.getClass().getName().startsWith(DEFAULT_HANDLER_PACKAGE_NAME)) {
                        LogUtils.e("IMPORTANT WARNING! You already have an UncaughtExceptionHandler, are you sure this is correct? If you use a custom UncaughtExceptionHandler, you must initialize it AFTER CustomActivityOnCrash! Installing anyway, but your original handler will not be called.");
                    }
                            ...
                            if (config.isEnabled()) {
                                ...
                                if (hasCrashedInTheLastSeconds(application)) {
                                    ...
                                    if (isStackTraceLikelyConflictive(throwable, errorActivityClass)) {
                                        LogUtils.e("Your application class or your error activity have crashed, the custom activity will not be launched!");
                                        if (oldHandler != null) {
                                            oldHandler.uncaughtException(thread, throwable);
                                            return;
                                        }
