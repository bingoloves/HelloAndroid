...
public class ShadowLayout extends FrameLayout {
    ...
    private void initAttributes(AttributeSet attrs) {
        ...
        try {
            ...
            //x轴偏移量
            mDx = attr.getDimension(R.styleable.ShadowLayout_shadowOffsetX, 0);
            //y轴偏移量
            mDy = attr.getDimension(R.styleable.ShadowLayout_shadowOffsetY, 0);
            mShadowColor = attr.getColor(R.styleable.ShadowLayout_shadowColor, Color.parseColor("#2a000000"));

            selectorType = attr.getInt(R.styleable.ShadowLayout_shapeMode, 1);
            ...
            Drawable trueBackground = attr.getDrawable(R.styleable.ShadowLayout_layoutBackgroundSelect);
            ...
            if (mBackGroundColor_true != -101 && layoutBackground != null) {
                throw new UnsupportedOperationException("使用了ShadowLayout_layoutBackgroundSelect属性，必须先设置ShadowLayout_layoutBackground属性。且设置颜色时，必须保持都为颜色");
            }

            if (layoutBackground == null && layoutBackground_true != null) {
                throw new UnsupportedOperationException("使用了ShadowLayout_layoutBackgroundSelect属性，必须先设置ShadowLayout_layoutBackground属性。且设置图片时，必须保持都为图片");
            }

            //边框颜色的点击
            stroke_color = attr.getColor(R.styleable.ShadowLayout_strokeColor, -101);
            stroke_color_true = attr.getColor(R.styleable.ShadowLayout_strokeColorSelect, -101);

            if (stroke_color == -101 && stroke_color_true != -101) {
                throw new UnsupportedOperationException("使用了ShadowLayout_strokeColorSelect属性，必须先设置ShadowLayout_strokeColor属性");
            }
            ...
            startColor = attr.getColor(R.styleable.ShadowLayout_start_color, -101);
            centerColor = attr.getColor(R.styleable.ShadowLayout_centerColor, -101);
            endColor = attr.getColor(R.styleable.ShadowLayout_end_color, -101);
            if (startColor != -101) {
                //说明设置了渐变色的起始色
                if (endColor == -101) {
            ...
            textColor = attr.getColor(R.styleable.ShadowLayout_textColor, -101);
            textColor_true = attr.getColor(R.styleable.ShadowLayout_textColorSelect, -101);
            text = attr.getString(R.styleable.ShadowLayout_text);
            text_true = attr.getString(R.styleable.ShadowLayout_textSelect);

            isClickable = attr.getBoolean(R.styleable.ShadowLayout_xclickable, true);
